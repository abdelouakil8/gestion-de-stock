...
def _render_barcode_image(value: str) -> QImage | None:
    if not barcode:
        logger.warning(
            "python-barcode not installed, skipping barcode image generation."
        )
        return None
    try:
...
def print_barcode_label(
    product: dict, printer: str | None, copies: int = 1
) -> Path | None:
    ...
    painter.drawText(
        0, 140, w, 60, Qt.AlignmentFlag.AlignCenter, f"{float(price):.2f} DZD"
    )
